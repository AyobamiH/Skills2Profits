...
//           <span className="text-bold bg-green-800 text-white text-sm">Hint</span> - Trading journal and analysis required later in the selection process.
//         </p>
//       </div>
...
const ApplicationForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    message: "",
    country: "",
    expertise: "", // New field for expertise
    agreement: false,
  });
  ...
  const isFormComplete = () => {
    const { name, email, message, phone, country, expertise, agreement } = formData;
    return (
      name.trim() !== "" &&
      email.trim() !== "" &&
      message.trim() !== "" &&
      phone.trim() !== "" &&
      country.trim() !== "" &&
      expertise.trim() !== "" && // Ensure expertise is selected
      agreement
    );
  };
    ...
    if (isFormComplete()) {
      ...
      try {
        ...
        setFormData({
          ...
          message: "",
          country: "",
          expertise: "", // Reset expertise field
          agreement: false,
        });
        setConfirmationMessage("Your message has been sent successfully!");
        setIsMessageSent(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <select
        ...
        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
      >
        <option className="" value="">Select your expertise</option>
      ...
      >
        <SendMessage
          ...
          disabled={!formData.agreement || isLoading}
        >
          {isLoading ? "Sending..." : "Submit & Start Earning"}
        </SendMessage>
      </motion.div>
    </form>
  );
